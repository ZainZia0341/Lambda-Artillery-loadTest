...
$stamp = [DateTime]::Now.ToString("yyyy-MM-dd_HH-mm-ss") + "Z"
$BaseOut = Join-Path "." ("monitoring-logs\" + $stamp) # <-- CHANGE THIS LINE
Ensure-Dir $BaseOut
...
if ($bedrockExists) {
  $rawEvents = Get-LogEvents -LogGroupName $BedrockLogGroup -StartTime $startQ -EndTime $endQ -Label "Bedrock" -MaxItems 200
    ...
    if ($rawEvents -and $rawEvents.Count -gt 0) {
      ...
      # Create CloudWatch Logs Insights formatted markdown
      $bedrockCols = @(
        '@timestamp', 'requestId', 'invocationTime', 'invocationId', 'outputModel', 'inferenceRegion',
